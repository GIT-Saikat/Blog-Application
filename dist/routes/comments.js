...
router.put("/:commentId", middleware, async (req, res) => {
    ...
    try {
        const found = await Client.comment.findUnique({ where: { id: commentId }, select: { author_id: true } });
        if (!found)
            return res.status(404).json({ message: "Comment not found" });
        if (found.author_id !== req.userId)
            return res.status(403).json({ message: "Invalid author" });
        const updated = await Client.comment.update({ where: { id: commentId }, data: { content: parsedData.data.content } });
